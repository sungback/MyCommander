...
- 구현 방식이 여러 개이고 UX, 데이터 안전성, API, 의존성, 아키텍처에 의미 있는 차이가 있으면 트레이드오프를 먼저 설명합니다.
- 더 단순하고 검증 가능한 접근이 있으면 요청 범위를 벗어나지 않는 선에서 역제안합니다.
- 변경한 코드 때문에 더 이상 쓰이지 않는 변수, 함수, import는 함께 제거합니다. 기존 데드 코드는 임의 삭제하지 않고 보고만 합니다.
- 모든 코드 변경은 사용자 요청, 저장소 증거, 테스트 목표 중 하나와 직접 연결되어야 합니다.
- 추측성 확장과 과도한 범용화는 피하되, 파일 손실, 권한, symlink/CloudStorage, 크로스플랫폼 경로처럼 이 앱의 핵심 위험은 반드시 검토합니다.
- 복잡한 작업은 `작업 단계 → 검증 명령 → 결과 확인` 단위로 나누어 진행합니다.
