...
- **Unicode Filename Policy:** Keep the current filename-creation behavior as-is. Do not add NFD normalization for new file/folder names. Korean filenames must remain compatible with the current NFC-preserving behavior so copies to Windows do not appear decomposed.
